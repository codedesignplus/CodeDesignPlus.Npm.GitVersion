...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.GitVersionOptions = void 0;
class GitVersionOptions {
    releaseBranch = 'main';
    ...
    majorIdIsRegex = false;
    minorIdIsRegex = false;
    releaseCandidateBranchSufix = 'rc';
    developmentBranchSufix = 'beta';
    defaultSufix = 'alpha';
    folder = process.cwd();
    previousVersion;
    newVersion;
    constructor(options) {
        this.releaseBranch = options.releaseBranch;
        this.releaseCandidateBranch = options.releaseCandidateBranch;
        this.betaBranch = options.betaBranch;
        this.majorIdentifier = options.majorIdentifier;
        this.minorIdentifier = options.minorIdentifier;
        this.prefix = options.prefix;
        this.dirAffected = options.dirAffected;
        this.folder = options.folder;
        this.previousVersion = JSON.parse(options.previousVersion);
        this.newVersion = JSON.parse(options.newVersion);
...
exports.GitVersionOptions = GitVersionOptions;
...
const semver_1 = __importDefault(__nccwpck_require__(5467));
...
class GitVersion {
    ...
    getPreviousTagAndVersion() {
        ...
        for (const tag of branchTags) {
            ...
            const currentVersion = semver_1.default.parse(tagWithoutPrefix);
            if (!currentVersion) {
                continue;
            }
            ...
            else if (semver_1.default.lt(previousVersion, currentVersion)) {
                previousVersion = currentVersion.version;
                previousTag = tag;
            }
            core.debug(`${tag} | previousTag = ${previousTag}, previousVersion = ${previousVersion}`);
        }
    ...
    getNewVersion() {
        ...
        for (const item of gitCommitsSince) {
            ...
            if (match) {
                newVersion = semver_1.default.inc(previousVersion, 'major');
                major = true;
                continue;
            }
        }
        if (!major) {
            for (const item of gitCommitsSince) {
                ...
                if (match) {
                    newVersion = semver_1.default.inc(previousVersion, 'minor');
                    continue;
                }
            }
        }
        ...
        if (currentBranch === this.options.releaseBranch) {
            ...
            const version = semver_1.default.parse(newVersion);
            if (version) {
                const versionWithoutPrerelease = `${version.major}.${version.minor}.${version.patch}`;
                newVersion = semver_1.default.inc(versionWithoutPrerelease, 'prerelease', prerelease.join('.'), false);
...
__exportStar(__nccwpck_require__(7207), exports);
